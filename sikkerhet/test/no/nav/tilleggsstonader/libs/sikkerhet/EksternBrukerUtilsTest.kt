...
internal class EksternBrukerUtilsTest {
    ...
    internal fun `skal kaste feil hvis det ikke finnes subject eller pid`() {
        setContextHolder()
        assertThatThrownBy { EksternBrukerUtils.hentFnrFraToken() }
            .hasMessage("Finner ikke pid på token")
    }

    @Test
    internal fun `kaster feil hvis sub finnes men ikke pid`() {
    ...
    private fun setContextHolder(sub: String? = null, pid: String? = null) {
        ...
        every { tokenValidationContext.getClaims("tokenx") } returns JwtTokenClaims(builder.build())
        every { tokenValidationContext.hasTokenFor("tokenx") } returns true
