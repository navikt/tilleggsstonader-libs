...
internal class FødselsnummerTest {
    ...
    internal fun `skal tillate helsyntetiske nummer fra dolly`() {
        ...
        listeAvBrukere.forEach {
            assertThat(it.fnr)
                .isEqualTo(Fødselsnummer(it.fnr).verdi)
                .withFailMessage { "Fødselsnummer ${it.fnr} er gyldig" }
            assertThat(it.dnr)
                .isEqualTo(Fødselsnummer(it.dnr).verdi)
                .withFailMessage { "Dnr ${it.dnr} er gyldig" }
            assertThat(it.fødselsdato)
                .isEqualTo(Fødselsnummer(it.fnr).fødselsdato)
                .withFailMessage { "Finner dato for ${it.fnr}" }
            assertThat(it.fødselsdato)
                .isEqualTo(Fødselsnummer(it.dnr).fødselsdato)
                .withFailMessage { "Finner dato for ${it.dnr}" }
            assertThat(false)
                .isEqualTo(Fødselsnummer(it.fnr).erDNummer)
                .withFailMessage { "${it.fnr} er ikke D-nummer" }
            assertThat(true)
                .isEqualTo(Fødselsnummer(it.dnr).erDNummer)
